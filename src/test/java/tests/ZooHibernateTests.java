...
class ZooHibernateTests {
    ...
    void insertIndexAnimal(Animal animal) {
        Throwable exception = assertThrows(PersistenceException.class, () -> dbHibernateService.insertAnimal(animal));
        org.assertj.core.api.Assertions.assertThat(exception.getMessage()).contains("Нарушение уникального индекса или первичного ключа");
    }
    ...
    void insertNullToWorkman() {
        ...
        Throwable exception = assertThrows(PersistenceException.class, () -> dbHibernateService.insertWorkman(workman));
        org.assertj.core.api.Assertions.assertThat(exception.getMessage()).contains("Значение NULL не разрешено для поля \"name\"");
    }
